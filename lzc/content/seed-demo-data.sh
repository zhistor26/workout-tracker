...
curl -sS -c "$COOKIE" -b "$COOKIE" -X POST "$BASE/user/signin" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=admin" \
  -d "password=admin" \
  -o /dev/null

curl -sS -c "$COOKIE" -b "$COOKIE" -X POST "$BASE/self/profile" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "language=zh-Hans" \
  -d "theme=browser" \
  -d "totals_show=running" \
...
for workout in /lzcapp/pkg/content/demo-data/*.gpx /lzcapp/pkg/content/demo-data/*.tcx /lzcapp/pkg/content/demo-data/*.zip; do
  [ -f "$workout" ] || continue
  curl -sS -c "$COOKIE" -b "$COOKIE" -X POST "$BASE/workouts" \
    -F "file=@${workout}" \
    -F "type=auto" \
    -F "notes=LazyCat demo import: $(basename "$workout")" \
    -o /dev/null
done
...
curl -sS -c "$COOKIE" -b "$COOKIE" "$BASE/open?file=/lzcapp/pkg/content/demo-data/morning-run.gpx" -o /dev/null
